		...
		return this.each(function (){
			...
			if (editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					if (e.keyCode === 13 && !e.shiftKey && (e.target.tagName === undefined || e.target.tagName != 'TEXTAREA')) {
				...
				switch (kdir) {
					...
					case 40 :	// DOWN
						if (p.iRow+1 <=  $t.rows.length-1) {
							var rec = $($t).jqGrid('getLocalRow', p.selrow);
							$($t).jqGrid("expandRow", rec);
							$($t).jqGrid("expandNode", rec);
							scrollGrid(p.iRow+1,p.iCol,'vd');
							$($t).jqGrid("editCell",p.iRow+1,p.iCol,false);
						}
					break;
					case 37 :
						if (p.iCol -1 >=  0) {
					...
					case 39 :	// RIGHT
						if (p.iCol +1 <=  p.colModel.length-1) {
							...
							scrollGrid(p.iRow,i,'h');
							$($t).jqGrid("editCell",p.iRow,i,false);
						}
					break;
//					case 13:
//						if (parseInt(p.iCol,10)>=0 && parseInt(p.iRow,10)>=0) {
//							$($t).jqGrid("editCell",p.iRow,p.iCol,true);
//						}
//					break;
					default :
						return true;
